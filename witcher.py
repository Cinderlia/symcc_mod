...
class Witcher():
    ...
    def find_all_php_files(self, directory_path):
            ...
            for root, dirs, files in os.walk(directory_path):
                for file in files:
                    if file.endswith('.php'):
                        ...
                        print(f"{WITCH_GO} Found PHP file: {full_path}")
        except Exception as e:
            print(f"{WITCH_FAIL} Error scanning directory {directory_path}: {e}")
                        ...
                        if self.jconfig.get("afl_inst_interpreter_binary", "").find("php-cgi") > -1:
                            ...
                            if target_path and os.path.exists(target_path):
                                if os.path.isdir(target_path):
                                    # 找到目录中的所有PHP文件
                                    php_files = self.find_all_php_files(target_path)
                                    for php_file in php_files:
                                        # 为每个PHP文件创建目标信息
                                        relative_path = os.path.relpath(php_file, self.appdir)
                                        ...
                                        print(f"{WITCH_GO} Added PHP file from directory: {php_file} -> {url_path}")
                                elif target_path.endswith('.php'):
                                    # 单个PHP文件
                                    php_targets.append({
                            ...
                            for php_target in php_targets:
                                ...
                                # 使用文件路径作为target_path，URL路径用于环境变量
                                effective_target_path = php_file_path
        
                                if not self.single_target or effective_target_path.find(self.single_target) > -1:
                                    if effective_target_path in targets_added:
                                        index = targets_added[effective_target_path]
                                        ...
                                        targets_added[effective_target_path] = len(trial["targets"])
                                        trial["targets"].append({
                                            "target_path": effective_target_path, 
                                            "url_path": php_url_path,  # 新增：保存URL路径
                                            "requests": [reqkey],
                                            "methods": {method: 1}, 
                                            "is_soapaction": is_soapaction,
                                            "last_completed_trial": -1, 
                                            "last_completed_refuzz": -1,
                                            "is_php_file": True  # 新增：标记为PHP文件
                                        })
                                        print(f"{WITCH_GO} Created fuzz target: {effective_target_path}")
...
#            dictionary_vars.append(b"%s'(&" % inputvar.encode("utf-8"))
    ...
    def start_fuzzer(self, do_resume, target_path, method_map, dictionary_str, seeds):
        ...
        # 在fuzz_campaign_status中查找当前目标的信息
        if hasattr(self, 'fuzz_campaign_status'):
            for trial in self.fuzz_campaign_status:
                for target in trial.get("targets", []):
                    if target["target_path"] == target_path:
                        current_target_info = target
                        break
                if current_target_info:
                    break
    
        # 设置SCRIPT_NAME环境变量
        if current_target_info and current_target_info.get("url_path"):
            # 使用专门存储的URL路径
            current_url_path = current_target_info["url_path"]
            os.environ["SCRIPT_NAME"] = current_url_path
            print(f"{WITCH_GO} Set SCRIPT_NAME to: {current_url_path}")
        else:
            # 回退到原始逻辑
            if target_path.startswith("http"):
                url_obj = urlparse(target_path)
                current_url_path = url_obj.path
                os.environ["SCRIPT_NAME"] = current_url_path
            else:
                # 对于文件系统路径，使用文件名作为SCRIPT_NAME
                current_url_path = f"/{os.path.basename(target_path)}"
                os.environ["SCRIPT_NAME"] = current_url_path
            print(f"{WITCH_GO} Set SCRIPT_NAME to (fallback): {current_url_path}")
    
        # 设置SCRIPT_FILENAME环境变量
        os.environ["SCRIPT_FILENAME"] = target_path
        print(f"{WITCH_GO} Set SCRIPT_FILENAME to: {target_path}")
    ...
    def copy_fuzzer_output_to_results(self, trial_index, target_path):
        ...
        # === 新增：查找目标的url_path用于生成目录名 ===
        url_path_for_naming = None
    
        # 在fuzz_campaign_status中查找当前目标的信息
        if hasattr(self, 'fuzz_campaign_status'):
            for trial_idx, trial in enumerate(self.fuzz_campaign_status):
                if trial_idx != trial_index:
                    continue
                for target in trial.get("targets", []):
                    if target["target_path"] == target_path:
                        # 优先使用url_path
                        url_path_for_naming = target.get("url_path")
                        break
                if url_path_for_naming:
                    break
    
        # 如果没有找到url_path，使用回退方案
        if not url_path_for_naming:
            if target_path.startswith("http"):
                # 对于HTTP URL，使用URL路径
                url_obj = urlparse(target_path)
                url_path_for_naming = url_obj.path
            else:
                # 对于文件路径，使用相对于appdir的路径
                if target_path.startswith(self.appdir):
                    url_path_for_naming = target_path.replace(self.appdir, '')
                else:
                    url_path_for_naming = f"/{os.path.basename(target_path)}"
    
        # 清理路径用于目录名
        if url_path_for_naming.startswith('/'):
            url_path_for_naming = url_path_for_naming[1:]
    
        # 替换特殊字符
        encoded_path = url_path_for_naming.replace('/', '+').replace('..', 'dotdot')
    
        # 限制目录名长度（避免过长的路径）
        if len(encoded_path) > 200:
            encoded_path = encoded_path[:200] + "_truncated"
    
        # 使用新的命名逻辑，但保留原有的 results_target_dir 调用结构
        targets_dir = f"tr{trial_index}_{encoded_path}"
        dst = os.path.join(self.report_dir, targets_dir)
    
        print(f"{WITCH_GO} Copy from {self.work_dir} to dst={dst}")
        print(f"{WITCH_GO} Using URL path for naming: {url_path_for_naming}")
    ...
    def copy_fuzzer_results_to_output(self, trial_index, target_path):
        # 使用与copy_fuzzer_output_to_results相同的命名逻辑
        url_path_for_naming = None
        ...
        targets_dir = f"tr{trial_index}_{encoded_path}"
        src = os.path.join(self.report_dir, targets_dir)
